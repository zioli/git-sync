...
func main() {
	flag.Parse()
	log.Println("[MAIN] REPOSITIORIO: %s", string(*flRepo))
	...
	for {
		...
		log.Println("done, lets go to the next iteration")
	}
}
...
func syncRepo(repo, dest, branch, rev string) error {
	...
	log.Println("[syncRepo] repo: %s", string(repo))
	log.Println("[syncRepo] dest: %s", string(dest))
	log.Println("[syncRepo] branch: %s", string(branch))
	log.Println("[syncRepo] rev: %s", string(rev))
	...
	log.Printf("git pull %q: %s", branch, string(output))
